...
import { reproducibilityBenchmarkS001 } from "@shared/reproducibilityBenchmark";
import { s001CanonicalFixture, scoreS001Run, type S001Score } from "@shared/reproducibilityFixture";
import {
  ArrowUpRight,
  BookOpen,
  Check,
...
export default function Home() {
  ...
  const [s001Result, setS001Result] = useState<S001Score | null>(null);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-canvas">
        ...
        <section id="reproducibility-benchmark" className="content-section benchmark-section"><div className="section-heading"><div><div className="eyebrow"><span className="eyebrow-line" />00B.8 / S-001 REPRODUCIBILITY BENCHMARK</div><h2>Compare by replay.<br /><em>Promote by proof.</em></h2></div><div className="section-intro">Un mismo escenario, inputs y fallo para CASTÚO y una implementación alternativa. El botón ejecuta sólo el fixture local; no crea evidencia de campo ni ventaja competitiva.</div></div><div className="benchmark-layout"><div className="benchmark-protocol"><div className="benchmark-status"><span className="repo-state repo-scoped">P2 / LOCAL FIXTURE</span><code>{reproducibilityBenchmarkS001.id}</code></div><h3>{reproducibilityBenchmarkS001.title}</h3><p>{reproducibilityBenchmarkS001.scenario.operationalTask}</p><div className="benchmark-failure"><span>FAILURE CONDITION</span><strong>{reproducibilityBenchmarkS001.scenario.failureCondition}</strong></div><div className="benchmark-artifacts">{s001CanonicalFixture.requiredArtifacts.map((artifact) => <span key={artifact}>{artifact}</span>)}</div><div className="benchmark-semantics"><span><strong>CASTÚO</strong> 1V · N3</span><span><strong>ALTERNATIVE</strong> ? · N3</span><small>1D declared · 1V reproducible · 1R independently reproduced</small></div><button className="button button-dark" onClick={() => setS001Result(scoreS001Run({ implementationId: "CASTUO-S001-A", evidence: "1V", events: [1, 2, 3].map((sequence) => ({ sequence, eventType: "CAPTURE" as const, payloadHash: `fixture:payload-00${sequence}`, provenanceRef: "commit:local-fixture" })), recoveryMs: 1200, artifactIds: [...s001CanonicalFixture.requiredArtifacts], independentlyReplayed: false }))}>{s001Result ? "Replay local complete" : "Run local fixture"} <ArrowUpRight size={14} /></button><small className="credential-footnote">{s001Result ? "LOCAL REPRODUCTION / NO FIELD CLAIM" : "No runtime or field result is asserted before execution."}</small></div><div className="benchmark-readout"><div className="benchmark-readout-head"><span>READOUT</span><strong>{s001Result?.claim ?? "BLOCKED"}</strong></div><div className="benchmark-metric-grid">{reproducibilityBenchmarkS001.metrics.map((metric) => <div className="benchmark-metric" key={metric.id}><span>{metric.id}</span><strong>{s001Result?.[metric.id] ?? "?"}{typeof s001Result?.[metric.id] === "number" ? "%" : ""}</strong><small>gate {metric.requiredFor}</small></div>)}</div><div className="benchmark-gates">{reproducibilityBenchmarkS001.gates.map((gate) => <span key={gate.id} className={gate.id === "P2" ? "gate-chip active" : "gate-chip"}>{gate.id} · {gate.id === "P2" ? "READY" : "BLOCKED"}</span>)}</div><div className="benchmark-claim-boundary"><span>CLAIM</span><strong>{s001Result?.claim ?? "BLOCKED"}</strong><span>COMPETITIVE ADVANTAGE</span><strong>{s001Result?.competitiveAdvantage ?? "NOT_ASSESSED"}</strong></div></div></div></section>
